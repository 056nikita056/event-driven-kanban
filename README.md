...
→ [web-production-ae3f0.up.railway.app](https://web-production-ae3f0.up.railway.app)

Проект представляет собой MVP event-driven канбан-доски для хакатона. Основная цель - показать архитектурный подход и работу ключевых сценариев: управление задачами, real-time синхронизацию, очередь событий, worker-обработчики, автоматизации и уведомления. 
...
- AI/regex-парсер быстрой задачи через Cmd+K. 
...
Важно: real-time работает через кастомный сервер [server.ts](server.ts). `npm run start` запускает именно его (`tsx server.ts`), а не стандартный `next start` — не перепутайте при ручном деплое.
...
Создайте второй Railway service из того же репозитория и укажите ему [railway.worker.toml](railway.worker.toml). Start command уже прописан в файле:
```bash
